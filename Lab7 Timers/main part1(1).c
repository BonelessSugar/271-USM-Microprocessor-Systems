...
void config_LED(void) {
  ...
  GPIOA -> PUPDR &= ~(3UL<<10); // no pull-up pull-down
	...
	TIM2 -> PSC = 3999; // pg1076/1903, pre-scaling clock by an amount (4Mhz/(PSC + 1)), 16-bit number, 65536 is max number
				// pg for x = 2 to 5
	TIM2 -> ARR = 499; // pg1018/1903, up-counting goes from 0 to auto-reload value
				// pg1077/1903
	TIM2 -> CCR1 = 99; // the value that the timer switches on, can be any value btw 0 and ARR
				// pg1078/1903
	TIM2 -> CCMR1 |= (3<<4);
				// pg1071 OC1M 0011, toggle mode, bit 4+5+6+16
